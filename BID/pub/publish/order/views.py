...
class BiddingInfoViewSet( viewsets.ModelViewSet, ListModelMixin):
...
def index(request):
	return redirect('/static/html/tender.html')

# 检验用户名是否存在
def verify_user(request):
    username = request.POST.get('username')
    phone = request.POST.get('phone')
    if username is None or phone:
	    username = phone
    try:
...
def send_code(request):
    ...
    print ('set session = '+request.session.get('smscode'))
...
def register_check(request):
    ...
    print('get session smscode =', request.session.get('smscode') )
        ...
        if password_confirm==password:
            try:
                ...
                if usr :
                    ...
                    return JsonResponse({'res': 0, 'message': 'exist!'})
                else:
                    ...
                    profile.save()
                    context['ok']=1
                    return JsonResponse({'res': 1, 'message': 'sucess!'})
            except:
                pass
        else:
            return JsonResponse({'res': 0 ,'message':err_password})
