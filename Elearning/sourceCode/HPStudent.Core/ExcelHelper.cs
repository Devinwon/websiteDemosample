     ...
     public static class ExcelHelper
     {
...

         {
...

             {
...

                 {
                     ...
                     OleDbDataAdapter odda = new OleDbDataAdapter(string.Format("SELECT * FROM [{0}A1:z2]", colName), conn);                    //("select * from [Sheet1$]", conn);

                     odda.Fill(ds, colName + "class");
                     OleDbDataAdapter odda1 = new OleDbDataAdapter(string.Format("SELECT * FROM [{0}A3:z5000]", colName), conn);
                     odda1.Fill(ds, colName);
                 }
         ...
         public static DataSet GetExcelToDataSet(string FileFullPath, bool no_HDR, string SheetName)
         {
...

             {
                 ...
                 DataSet ds = new DataSet();
                 OleDbDataAdapter odda = new OleDbDataAdapter(string.Format("SELECT * FROM [{0}A1:F5000]", SheetName), conn);                    //("select * from [Sheet1$]", conn);
         ...
         ///Author(作者)：Sean.j
         ///Created Date(创建日期):2016-03-31 08:55:10
         ///Update Date(更新日期): 2016-03-31 08:55:10
         /// </summary>
         /// <param name="FileFullPath">Excel表服务器上的全路径</param>
         /// <param name="no_HDR">首行是否作为表头显示 True为不做表头</param>
         /// <param name="SheetName">表单名称</param>
         /// <param name="StartDataCellFiled">数据域表格开始单元格，通常为A1</param>
         /// <param name="EndDataCellFiled">数据域表格结束单元格，可自定义如K3000；Z5000等</param>
         /// <returns>
         /// 返回选中该区域例如A1:Z5000区域的表格数据返回数据集
         /// </returns>
         public static DataSet GetExcelToDataSet(string FileFullPath, bool no_HDR, string SheetName, string StartDataCellFiled, string EndDataCellFiled)
         { 
             try{
                 string strConn = GetExcelConnectionString(FileFullPath, no_HDR);
                 OleDbConnection conn = new OleDbConnection(strConn);
                 conn.Open();
                 DataSet ds = new DataSet();
                 OleDbDataAdapter odda = new OleDbDataAdapter(string.Format("SELECT * FROM [{0}{1}:{2}]", SheetName, StartDataCellFiled, EndDataCellFiled), conn);                    //("select * from [Sheet1$]", conn);
                 odda.Fill(ds, SheetName);
                 conn.Close();
                 return ds;
             }
