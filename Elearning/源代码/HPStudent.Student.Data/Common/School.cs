...
namespace HPStudent.Student.Data.Common
{
    public class SchoolCommon
    {
        ...
        /// <returns></returns>
        public static List<HPStudent.Student.ViewModel.Student.StudentSchool> GetSchoolByAreaID(string ParentAID, string AreaID, int start, int length, out int TotalRows)
        {
            ...
            List<HPStudent.Student.ViewModel.Student.StudentSchool> ViewSchoolList = new List<HPStudent.Student.ViewModel.Student.StudentSchool>();
            ...
            using (SqlDataReader reader = SqlHelper.ExecuteReader(CommandType.Text, sql, paras))
            {
                while (reader.Read())
                {
                    ...
                    ViewSchoolList.Add(
                       new HPStudent.Student.ViewModel.Student.StudentSchool()
                       {
        ...
        {
            string sql = @"select * from Common_Area as ca where  exists(
	select ParentAID from Common_Area ca1 where AreaName=@AreaName and ca1.ParentAID=ca.AreaID)";
            SqlParameter[] paras = new SqlParameter[] { 
                new SqlParameter("@AreaName", AreaName),                       
            };
            HPStudent.Entity.Common_Area Common_Area = new HPStudent.Entity.Common_Area();
            using (SqlDataReader reader = SqlHelper.ExecuteReader(CommandType.Text, sql, paras))
            {
                while (reader.Read())
                {
                    Common_Area.ParentAID = Convert.ToInt32(reader["ParentAID"].ToString());
                    Common_Area.AreaID = Convert.ToInt32(reader["AreaID"].ToString());
                    Common_Area.AreaName = reader["AreaName"].ToString();
                    Common_Area.DisplayOrder = Convert.ToInt32(reader["DisplayOrder"].ToString());
                    Common_Area.Level = Convert.ToInt32(reader["Level"].ToString());
